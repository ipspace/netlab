...
* Add 'retry tests until the timeout' functionality to 'netlab validate'
* Use SSH_CONNECTION local IP address in when displaying tool launch URL
* Warn user about IBGP sessions on nodes that have no IGP
* Display original system default path settings with 'netlab show defaults' command
...
* Optimize Cisco IOS BGP configuration templates to improve convergence times
* Cisco IOS XRd container support
* Use standard Linux 'lo' loopback on FRR instead of 'lo0' dummy interface
* Execute a series of integration tests with the `device-module-test` script
...
* Use 'retry actions' validation functionality in OSPF and BGP integration tests
...
* Stop 'netlab down' complaints with double '--force' option
* Missing BGP prefix from a stub network in IPv6 BGP RR integration test
* Arista EOS wants to have OSPFv3 configured on interfaces first
...
* 'netlab status' should return error status if no labs are running
...
* Bug fix: FRR needs router-id in VRF BGP in IPv6-only deployments
