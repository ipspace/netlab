...
* Implement BGP route server/client in bgp.session. Available on Arista EOS, Bird, Cisco IOSv/IOS-XE, Cumulus Linux, and FRR.
* Support Ubuntu 22.04
* Add warnings/confirmations to **netlab install** describing Python package installation options
* Add `--user` option to the **netlab install** command to install Python packages in user `.local` directory
...
FRR:
* Load VRF kernel module during initial configuration, and disable management VRF when not available (#1210)
...
VyOS:
* Support for bgp.policy plugin (#1195)

(release-1.8.3-dev)=
**New development tools:**

* bgp.policy integration tests
* Implement Arista EOS and Cumulus Linux BGP validation plugins, add integration tests for validation plugins
* Refactor FRR BGP validation plugin
* Validation plugins can Raise successful results with log.Result
...
VyOS:
* Restart FRR after changing VyOS hostname
