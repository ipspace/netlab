...
* _netlab_ reports an error (that can be turned off) when you try to connect nodes with VLAN forwarding mode **irb** or **bridge** to routed VLANs. This might break some creative topologies; you can disable the check with a **defaults** setting described in the error message.
* The default value of **vrrp.preempt** parameter was not well-defined, and each device used a device-specific default. The default value in release 1.9.2 is *True* and is used in the device configuration.
* You can no longer set [the VLAN interface parameters](vlan-interface-parameters) on the first VLAN access link (that practice was unsupported and discouraged but still allowed). The VLAN interface parameters have to be set in the node **vlans** dictionary.
