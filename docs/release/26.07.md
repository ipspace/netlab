...
* VPP is a regular containerlab device (no longer classified as a daemon). Pin the FD.io release with **defaults.devices.vpp.clab.sw_version** before running **[netlab clab build vpp](netlab-clab-build)** — see [](build-vpp).
