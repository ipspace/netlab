...
* Deploy initial device configurations[^itag]
* Deploy module-specific device configurations[^mtag]
* Deploy configurations generated by the custom configuration templates[^ctag]

[^norm]: Usually: shut down interfaces in the default VLAN on devices pretending to be stupid bridges
[^rtag]: Use the `--ready` flag to execute just this step
[^ntag]: Use the `--normalize` flag to execute just this step
[^itag]: Controlled by `-i` flag
[^mtag]: Controlled by `-m` flag
[^ctag]: Controlled by `-c` flag

Device configurations prepared by the **netlab create**/**netlab up** commands, or created as the first step of the **netlab initial** process, are used with the **_device_\_config** Ansible modules to configure most devices. Sometimes, the configuration task list includes additional tasks[^init]. Some Linux-based containers are configured with shell scripts that run before **netlab initial** starts the Ansible playbook.
...
```text
usage: netlab initial [-h] [--log] [-v] [-q] [-i] [-n] [-m [MODULE]] [-l LIMIT] [-c]
                      [--ready] [--fast] [-o [OUTPUT]] [--clean] [--dry-run]
                      [--instance INSTANCE]
...
options:
  ...
  -n, --normalize       Deploy just the normalize configuration
  -m, --module [MODULE]
  ...
  --clean               Clean up the output directory before creating the initial
                        configuration
  --dry-run             Print actions that would be executed without executing them
  --instance INSTANCE   Specify the lab instance to configure
...
The initial device configurations are created in per-node subdirectories of the `node_files` directory. Use **netlab create** or **netlab initial -o**[^oval] to create the device configurations without deploying them.

You can also use the `-v` flag (multiple times) to display internal and Ansible logging messages.

[^oval]: The optional value of `-o` parameter specifies the output directory name (default: `config`)
