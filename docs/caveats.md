...
* **Static VXLAN flooding is configured correctly but its BUM datapath is not exercised by the container image.** With *netlab*'s default `vxlan.flooding: static`, `netsim/ansible/templates/vxlan/arcos.j2` renders ArcOS's static-VXLAN model -- the global `overlay static-vxlan` knob plus an `L2VLAN_AWARE_BUNDLE` network-instance carrying `static-vxlan remote-vteps` and the VNI-to-VLAN mapping (ArcOS keeps the remote-VTEP list there, not on the per-VLAN `L2VLAN` instance the EVPN path uses; the two modes are mutually exclusive). Everything the device reports is correct: the tunnel resolves (`show overlay static-vxlan-tunnels`), the VNIs come up `BRIDGED` with the right `static-rteps`, and RIB and FIB both hold the tunnel with `flood-and-learn true`. The image, however, never programs the head-end-replication entry into the kernel FDB, so the node can answer traffic whose source it has already learned from an inbound VXLAN packet but can never *initiate* a flood -- an ARP it has to send across the tunnel itself is lost. Adding the entries by hand (`bridge fdb append 00:00:00:00:00:00 dev vxlan<vni> dst <rtep>`) makes the affected tests pass with no configuration change. **EVPN flooding is unaffected** -- the same image programs both the flood entry and the remote MACs from EVPN type-2/type-3 routes -- so prefer `vxlan.flooding: evpn` on ArcOS.
