...
* EVPN Symmetric IRB is supported only from the Aruba AOS-CX Virtual version *10.13*. Additionally:
  ...
  * Manually Setting Virtual-MAC for EVPN RT-5 is not working (but autogenerated one is fine).
  * It seems there are some interop issues on Centralized IRB topologies (a topology with only AOS-CX devices works fine, mixed *linux-based* devices with AOS-CX acting as *Central IRB* is broken).
