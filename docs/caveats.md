...
* **Static VXLAN flooding is configured correctly, but the container image never programs the head-end-replication entry, so the node can never *initiate* a BUM flood.** With *netlab*'s default `vxlan.flooding: static`, `netsim/ansible/templates/vxlan/arcos.j2` renders ArcOS's static-VXLAN model -- the global `overlay static-vxlan` knob plus an `L2VLAN_AWARE_BUNDLE` network-instance carrying `static-vxlan remote-vteps` and the VNI-to-VLAN mapping (ArcOS keeps the remote-VTEP list there, not on the per-VLAN `L2VLAN` instance the EVPN path uses). Everything the device reports is correct: the tunnel resolves (`show overlay static-vxlan-tunnels`), the VNIs come up `BRIDGED` with the right `static-rteps`, RIB and FIB hold the tunnel with `flood-and-learn true`, and the L2RIB holds a resolved inclusive-multicast entry for the remote VTEP. What never happens is the final step: no `00:00:00:00:00:00 dst <rtep> self` entry is programmed into the kernel FDB. **Receive works -- data-plane learning from inbound VXLAN packets is fine** -- so the node answers traffic whose source it has already learned, but an ARP or any other BUM frame it has to originate across the tunnel is lost. Adding the entries by hand (`bridge fdb append 00:00:00:00:00:00 dev vxlan<vni> dst <rtep>`, after the tunnel shows `RESOLVED true` -- ArcOS discards them if added earlier) makes the affected tests pass with no configuration change. **EVPN flooding is unaffected**: the same image programs both the flood entry and the remote MACs from EVPN type-2/type-3 routes. Switching a topology over is more than flipping the flag -- *netlab* only accepts `vxlan.flooding: evpn` on a node that also runs the `evpn` module (and therefore `bgp`), so it means adding an EVPN control plane, not changing one attribute.
