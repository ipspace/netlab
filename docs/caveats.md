...
* **First-hop redundancy (VRRP) needs a container bootstrap, and its IPv4 checksum does not interoperate.** `netsim/ansible/templates/gateway/arcos.j2` renders VRRP for **both** IPv4 and IPv6 -- ArcOS hangs the group off the interface address of the matching address family and exposes the identical leaf set under each (`virtual-address`, `priority`, `advertisement-interval`, `accept-mode`). Two device quirks shape the result:
  * The ArcOS process manager (`spyder`) starts `vrrp.bin` lazily, on the first VRRP commit, and the daemon inherits `spyder`'s file-descriptor **soft** limit. Docker hands the container a 1048576 `nofile` limit, systemd caps `spyder` at 4096, and the daemon's LTTng-UST tracing library indexes a 1024-bit `fd_set` with that number, so `vrrp.bin` aborts on startup (*bit out of range 0 - FD_SETSIZE on fd_set*, signal 6) and `spyder` disables it after three retries. The device then commits the VRRP configuration, shows it in the running configuration, and never runs the protocol -- and `show interface <if>` reports `application communication failure` for any interface carrying a VRRP group. containerlab has no per-node `ulimit` knob, so *netlab* lowers the soft limit on the already-running `spyder` from `netlab_start_exec`; only daemons `spyder` forks afterwards (VRRP is the only lazily-started one) inherit it. With that in place VRRP converges: the DUT wins the election, forwards traffic sent to the virtual IP/MAC, and the IPv6 master/backup/preempt transitions all work.
