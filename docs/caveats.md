...
* **A discard static route aimed at a directly-connected host stops working once that host is resolved.** `features.routing.static.discard` is declared and the configuration is correct -- `next-hop DROP` commits in the global table and in a VRF, for IPv4 and IPv6, and installs as `blackhole <prefix> proto static`. It also *works*, until ArcOS resolves the neighbour entry for the target: as soon as the host sends anything to the device (ARPing its default gateway is enough), the connected-host (ADJACENCY) route replaces the discard and the destination becomes reachable again. Verified on a single lab, sampling either side of the transition: with no neighbour entry, `blackhole 172.16.42.42 proto static` is in the FIB and a remote host gets 100% packet loss; after the target pings its gateway, `ip neigh` shows it `REACHABLE`, **no** blackhole entry for that prefix remains, `ip route get` returns `172.16.42.42 dev swp2 src 172.16.42.1`, and the same ping gets 0% loss. A discard for a prefix that is **not** directly connected is unaffected and blackholes normally. The effect is therefore time- and traffic-dependent rather than a clean configuration failure, and waiting longer makes it *more* likely to bite -- `tests/integration/routing/25-static-discard.yml` aims all four of its discards at hosts on connected LANs, which is why its results move around on this platform.
