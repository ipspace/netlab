...
* **First-hop redundancy (VRRP) is configuration-only on this container image.** `netsim/ansible/templates/gateway/arcos.j2` renders VRRP for **both** IPv4 and IPv6 -- ArcOS hangs the group off the interface address of the matching address family and exposes the identical leaf set under each (`virtual-address`, `priority`, `advertisement-interval`, `accept-mode`) -- and the configuration commits cleanly. The image, however, ships **no VRRP process**: an FRR neighbour configured with a *lower* priority stays Master and reports **zero** advertisements received from ArcOS, and `show interface <if>` returns `application communication failure` as soon as a VRRP group is configured on it (the same interface reads back normally without one). So `gateway/02-vrrp` and `gateway/03-vrrp-vlan` deploy and commit but do not converge. `gateway.vrrp.preempt` is not rendered: this image has no `preempt` leaf under `vrrp-group` in either address family (`preempt`, `preempt enabled` and `preempt-mode` are all rejected with *element does not exist*), and preemption is on by default. **Anycast gateways are not supported** -- nothing renders them -- and are no longer declared in `features.gateway.protocol`.
