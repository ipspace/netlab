...
* **Multi-area OSPF: adjacencies and routing work, but the ABR mis-summarizes /32 host routes.** An ArcOS OSPF instance accepts interfaces in several areas at once, forms adjacencies in each, and floods correct inter-area Type-3 summaries for transit (e.g. /30) networks -- verified live. However, when the ABR summarizes a **/32 loopback host route** into another area it emits the Type-3 LSA with a **/0 network mask** (a platform bug: a neighbour even mis-installs it as a `0.0.0.0/0` default). So a loopback placed in a **non-backbone** area is not reachable from other areas; loopbacks in the backbone (area 0), and all inter-area transit prefixes, are unaffected. IPv4 and IPv6 (OSPFv3) behave the same.
