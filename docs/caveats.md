...
* **BGP sessions do not come up until the device has finished booting.** The ArcOS BGP daemon starts with the container and rejects *every* session FSM transition -- logging `bgp_send_open: Nbr <ip> failed to change to OPENSENT` and `bgp_receive_open: Nbr <ip> failed to changed to OPENCONFIRM` a few times a second -- until it logs `BGP startup config has been read`, which it does once, a few minutes after the container starts. Peers see the ArcOS node stuck in `Active`/`OpenConfirm` for that whole window. This is not a protocol timer (the neighbour connect-retry is 10 seconds) and not a configuration-ordering problem: *netlab* commits the BGP configuration well before the barrier lifts, and the sessions come up within a second of it lifting regardless of when the commit happened. Past that point BGP behaves normally -- a neighbour added to a running device reaches ESTABLISHED in under six seconds. The residual wait after `netlab up` returns is around 70 seconds; it is declared to the validation framework as `arcos.ebgp_session` / `arcos.ibgp_session` in `tests/integration/wait_times.yml` rather than papered over by changing protocol timers. Only BGP is affected: the OSPF and IS-IS processes are spawned when their configuration is committed, so they are not subject to it.
