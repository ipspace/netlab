...
* On the bright side, *Virtual Network* usage can be disabled by setting *defaults.devices.dellos10.group_vars.use_virtual_networks* to `False`; this enables some features like running OSPF and VRRP on traditional VLAN SVI interfaces (though without VXLAN/EVPN)
