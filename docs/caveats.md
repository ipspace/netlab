...
* **BGP sessions come up roughly 70 seconds after the rest of the configuration.** The ArcOS BGP daemon rejects every session FSM transition until it finishes reading its boot-time startup configuration, which it does once, around three minutes after the container starts; peers see the ArcOS node in `Active`/`OpenConfirm` until then. This is a device startup barrier, not a protocol timer, so *netlab* changes no timers -- it declares the wait as `arcos.ebgp_session` / `arcos.ibgp_session` in `tests/integration/wait_times.yml`. Only BGP is affected: the OSPF and IS-IS processes are spawned when their configuration is committed.
...
* **First-hop redundancy (VRRP) needs a container bootstrap, and its IPv4 checksum does not interoperate.** `netsim/ansible/templates/gateway/arcos.j2` renders VRRP for **both** IPv4 and IPv6 -- ArcOS hangs the group off the interface address of the matching address family and exposes the identical leaf set under each (`virtual-address`, `priority`, `advertisement-interval`, `accept-mode`). Two device quirks shape the result:
  * **The VRRP daemon does not start unless the file-descriptor soft limit is put back to the systemd default.** The ArcOS process manager (`spyder`) starts `vrrp.bin` lazily, on the first VRRP commit, and the daemon inherits `spyder`'s file-descriptor **soft** limit. `spyder.service` sets no `LimitNOFILE`, so systemd starts `spyder` at its default soft limit of 1024 -- but `/usr/sbin/spyder` is a Python program that raises its *own* soft limit to 4096 in `main()` (`resource.setrlimit`, leaving the hard limit alone). `vrrp.bin` inherits that 4096 and is linked against `liblttng-ust`, whose fd tracker indexes a 1024-entry `fd_set` with the inherited limit, so it aborts in its constructor (*bit out of range 0 - FD_SETSIZE on fd_set*, signal 6; the same code path also reports *fd-tracker.c: `lttng_ust_add_fd_to_tracker`: Assertion `IS_FD_VALID(fd)` failed*). `spyder` disables the daemon after three retries, so the device commits the VRRP configuration, shows it in the running configuration, and never runs the protocol -- and `show interface <if>` reports `application communication failure` for any interface carrying a VRRP group. containerlab has no per-node `ulimit` knob, so *netlab* puts the **soft** limit back to systemd's default on the already-running `spyder` from `netlab_start_exec`, and only processes `spyder` forks afterwards inherit it. Nothing ends up fd-starved: ArcOS daemons raise their own `RLIMIT_NOFILE` once running (`vrrp.bin` settles at 4096 soft / 8192 hard), so the restored value governs only the window between `fork` and the daemon's own `setrlimit` -- exactly the window the `liblttng-ust` constructor runs in. (`spyder.service` has no `Restart=`, so the adjustment cannot be silently undone mid-lab.) With that in place VRRP converges: the DUT wins the election, forwards traffic sent to the virtual IP/MAC, and the IPv6 master/backup/preempt transitions all work.
  * **IPv4 VRRP does not interoperate with other *netlab* devices.** ArcOS follows RFC 5798 and computes the IPv4 VRRPv3 checksum over the IPv4 pseudo-header, with no knob to disable it (there is no such leaf under `vrrp-group`). *netlab* configures every other VRRP device the other way -- `no vrrp <g> checksum-with-ipv4-pseudoheader` on FRR, `vrrp <g> ipv4 checksum pseudo-header exclude` on EOS, `ipv4 checksum exclude-pseudo-header` on EXOS -- so ArcOS and its peer discard each other's IPv4 advertisements and both stay Master (verified: turning the pseudo-header back on in FRR moves it to Backup within one advertisement interval). A *single* ArcOS first-hop router is unaffected -- it becomes Master and forwards -- but IPv4 **election** against a non-ArcOS peer does not work. **IPv6 is unaffected**, because the pseudo-header is mandatory there for every implementation.

  `gateway.vrrp.preempt` is not rendered: this image has no `preempt` leaf under `vrrp-group` in either address family (`preempt`, `preempt enabled` and `preempt-mode` are all rejected with *element does not exist*), and preemption is on by default. **Anycast gateways are not supported** -- nothing renders them -- and are not declared in `features.gateway.protocol`.
* **Multi-area OSPF: adjacencies and routing work, but the ABR mis-summarizes /32 host routes.** An ArcOS OSPF instance accepts interfaces in several areas at once, forms adjacencies in each, and floods correct inter-area Type-3 summaries for transit (e.g. /30) networks -- verified live. However, a **loopback host route** is summarized with the wrong prefix length, and the two address families get it wrong differently:

  * **OSPFv2** emits the Type-3 LSA with a **/0 network mask**. Read from a neighbour's database, the ABR's own summary LSA for its `10.0.0.1/32` loopback is `Link State ID: 10.0.0.1 ... Network Mask: /0` -- and a neighbour mis-installs it as a `0.0.0.0/0` default. The same LSA set carries the /30 transit prefixes with the correct `Network Mask: /30`, so it is specific to host routes.
  * **OSPFv3** advertises the loopback with the **interface prefix length instead of /128** -- a neighbour installs `2001:db8:1:1::/64` where `2001:db8:1:1::1/128` is expected. The prefix is present, just not as a host route, which is a quieter failure than the IPv4 form.

  This also happens **inside a VRF**: a per-VRF OSPF instance acting as an ABR shows the identical `Network Mask: /0` for the VRF loopback while summarizing the VRF's transit prefixes correctly.

  So a loopback placed in a **non-backbone** area is not reachable from other areas; loopbacks in the backbone (area 0), and all inter-area transit prefixes, are unaffected.
* **OSPF default-route origination is not declared, but the platform can originate a *conditional* default.** `features.ospf.default` is deliberately absent, so _netlab_ refuses `ospf.default` with *"cannot originate ospf default route"* -- a message that **understates the platform**. ArcOS has no `default-information originate` under OSPF: every form of `network-instance <ni> protocol OSPF <inst> ... default-information ...` is rejected with *element does not exist*, the whole OSPF instance subtree is only `area` and `global`, and unlike BGP it has no `afi-safi` container to hang the leaf on. What it does have is ordinary redistribution -- a `table-connection STATIC OSPF <af>` carries `0.0.0.0/0` / `::/0` like any other prefix, in contrast to the BGP table-connection, which the ArcOS CLI guide's *BGP Default-Information Originate* section states excludes the default route. **Verified live in a neighbour's routing table, in both the global table and a VRF, IPv4 and IPv6**: an FRR peer installed `O>* 0.0.0.0/0` and `O>* ::/0` from a genuine Type-5 AS-external LSA (`Network Mask /0`, metric type 2, metric 10). What blocks _netlab_ is the **model mismatch, not the platform**. `ospf.default.always` has no equivalent -- origination is strictly conditional on a default route being present in the RIB, proven by re-creating the table-connection with no `0.0.0.0/0` in the RIB and getting only the control prefix. `ospf.default.cost` and `.type` are accepted and then **break redistribution**: an import policy carrying `ospf-actions set-metric` commits, reads back correctly in `show running-config routing-policy`, and makes *every* external LSA disappear while the device still reports `asbr-router true` (A/B'd twice; the emitted default is always E2 / metric 10). Since `netsim/modules/_routing.py` gates only `default.policy` against the device feature and passes `always`, `cost` and `type` through ungated, declaring `ospf.default` would convert six clean refusals into six failures -- so it stays undeclared until the feature flag can express the supported subset.
* **IS-IS IPv6 is single-topology, not multi-topology (RFC 5120).** This ArcOS build has no multi-topology knob (no such leaf in the OpenConfig IS-IS model; every `... multi-topology true` form is rejected on commit), so it advertises IPv6 prefixes in the standard IPv6 Reachability TLV rather than the MT IPv6 Reachability TLV. _netlab_ configures every other IS-IS device with multi-topology for IPv6, and an MT neighbour will **not** install ArcOS's IPv6 routes (verified: an FRR neighbour formed the adjacency and installed ArcOS's IPv4 loopback but showed no ArcOS IPv6 route). **IPv4 IS-IS interoperates normally; IPv6 IS-IS works only in an all-ArcOS IS-IS domain** — cross-vendor dual-stack / IPv6-only IS-IS is unsupported. This is why the *IS-IS MT* column for ArcOS is ❌ in the IPv6 support table. OSPFv3, BGP IPv6, and multi-area OSPF are unaffected (they interoperate cross-vendor).
* **Static VXLAN flooding is configured correctly, but the container image withdraws the head-end-replication entry moments after programming it, so a lab comes up unable to *initiate* a BUM flood.** With *netlab*'s default `vxlan.flooding: static`, `netsim/ansible/templates/vxlan/arcos.j2` renders ArcOS's static-VXLAN model -- the global `overlay static-vxlan` knob plus an `L2VLAN_AWARE_BUNDLE` network-instance carrying `static-vxlan remote-vteps` and the VNI-to-VLAN mapping (ArcOS keeps the remote-VTEP list there, not on the per-VLAN `L2VLAN` instance the EVPN path uses). Everything the device reports is correct: the tunnel resolves (`show overlay static-vxlan-tunnels`), the VNIs come up `BRIDGED` with the right `static-rteps`, RIB and FIB hold the tunnel with `flood-and-learn true`, and the L2RIB holds a resolved inclusive-multicast entry for the remote VTEP. What does not survive is the kernel FDB entry. Watching `bridge fdb show dev vxlan<vni>` through a lab start (5-second samples, reproduced on two independent runs): the tunnel resolves, ArcOS programs `00:00:00:00:00:00 dst <rtep> self` about 30 seconds later, and then **removes it again some 15 seconds after that**, leaving none for the rest of the lab's life. **Receive still works** -- data-plane learning from inbound VXLAN packets is fine -- so the node answers traffic whose source it has already learned, but an ARP or any other BUM frame it has to originate across the tunnel is lost.
  Two operator workarounds, both verified, neither available to a configuration template: **flap the underlay link towards the remote VTEP** (any unresolved-to-resolved tunnel transition makes ArcOS program the entries again, and after a flap they persist -- a lab in that state re-validates `vxlan/03-vxlan-irb` at 9/9 with nothing added by hand); or add them yourself with `bridge fdb append 00:00:00:00:00:00 dev vxlan<vni> dst <rtep>`, which is accepted whether or not the tunnel is currently resolved. Add them **after** the lab has settled: entries added during the start-up window are removed along with ArcOS's own by the withdrawal described above.
  **EVPN flooding is unaffected**: the same image programs both the flood entry and the remote MACs from EVPN type-2/type-3 routes, and keeps them. Switching a topology over is more than flipping the flag -- *netlab* only accepts `vxlan.flooding: evpn` on a node that also runs the `evpn` module (and therefore `bgp`), so it means adding an EVPN control plane, not changing one attribute.
* **EVPN symmetric IRB / L3VNI: type-5 routes are originated, but never advertised to the peer.** The earlier claim that a route-type-5 prefix "is never originated" was wrong -- it was measured before the VRF BGP instance carried its connected subnets, so there was nothing to originate. With those present, `show overlay vni-all <transit_vni>` reports `vni-run-state UP` and `vni-type ROUTED`, and the default instance's `L2VPN_EVPN` table holds one well-formed type-5 route per connected subnet in the VRF -- RD-qualified, with `encap-type:VXLAN` and a per-VTEP `router-mac`. The **import** direction works once the route target is written explicitly: ArcOS auto-derives the IP-VRF route target as `<AS>:<L3VNI>` while *netlab* (and FRR) derive it as `<AS>:<vrf-id>`, so nothing matched until the netlab value was configured under the unicast AFI in an `rt-afi-safi L2VPN_EVPN` container, per address family; with it, the peer's subnet appears in the VRF and inter-subnet traffic to it flows. What does **not** work is the egress half: the locally originated type-5 routes carry `path-types [ BEST_PATH EXPORTED_PATH ]` and **no rib-out entry**, so the peer never receives them and its EVPN table holds only its own. `rt-afi-safi L2VPN_EVPN evpn-route-options suppress-rt5-routes false` commits and changes nothing. Symmetric IRB therefore stays undeclared -- only the L2VNI / distributed-anycast-gateway form is -- but the gap is one step narrower, and in a different place, than previously recorded. Reported to Arrcus.

* **A discard static route aimed at a directly-connected host is displaced by the connected-host route whenever that host's neighbour entry is `REACHABLE`.** `features.routing.static.discard` is declared and the configuration is correct -- `next-hop DROP` commits in the global table and in a VRF, for IPv4 and IPv6, and installs as `blackhole <prefix> proto static`. The RIB keeps the discard as a candidate (`origin STATIC static` with `interface null0`) but reports `best-protocol ADJACENCY` for the same /32, and the blackhole is withdrawn from the FIB. The correlation is one-to-one, per target and per address family: on a settled lab the only discard still installed was the one whose target had **no** neighbour entry, while every target showing `REACHABLE` had none. It is sticky -- shutting the target host's interface for 90 s left the neighbour `REACHABLE`, `best-protocol` at `ADJACENCY` and no blackhole, and deleting the neighbour entry by hand is reinstated immediately. A discard for a prefix that is **not** directly connected is unaffected and blackholes normally. **`tests/integration/routing/25-static-discard.yml` aims all four of its discards at hosts on connected LANs and cannot be made reliably green on this platform by any `wait:` value**: for roughly the first three minutes after the DUT starts the ArcOS STATIC daemon has installed nothing, so the discards do not exist; then all four appear for some 16-21 seconds while the targets are still `STALE`; then the targets go `REACHABLE` and the blackholes leave the FIB for good. Both ends of that timeline fail the test and only the narrow middle passes -- measured on five fresh labs, all five reported 7/11.

* **EVPN and static VXLAN flooding cannot be combined on ArcOS.** `overlay static-vxlan` is a global flood-and-learn mode, and the device rejects the EVPN configuration while it is on (*"'network-instance advertise-mac-routes' must not be true when 'overlay static-vxlan' is true"*). *netlab* core permits the combination, so `netsim/devices/arcos.py` refuses it during topology transformation rather than letting `netlab initial` die halfway through with a raw vendor error. Set `vxlan.flooding: evpn` on the node, or drop the `evpn` module from it.
* **Inter-VRF route leaking is not rendered, and the RT/L3VPN form was measured -- not assumed -- as not working on this image.** The form *netlab*'s `vrfs.<name>.import/export` model maps onto is BGP L3VPN route targets, and its **export** half works completely: an L3VRF BGP instance takes `global route-distinguisher`, route targets go under `global afi-safi <AF>_UNICAST rt-afi-safi L3VPN_IPV4_UNICAST route-target <rt> both`, and with a per-VRF `label-allocation-mode` plus an MPLS `reserved-label-block ... usage BGP_COMMON protocol-identifier BGP` the VRF reaches `l3vpn-ipv4-export-readiness: "Export ready"` and the per-VRF `exported-rib` fills with correctly targeted, labelled VPN routes (`65000:2:172.18.1.0/24`, `local-label 100000`, `ext-community route-target:65000:2`). Finding that label block is the step everything else waits on: without it the VRF sits at *"Label/SID allocation mode not configured"* and then *"IPv4 AF in this network-instance has no deagg/instance label/sid"*, and the label manager logs `LibLabelAlloc<BGP_0>: Usage not found`.

  The **import** half never fires for a locally exported route. The default instance's `L3VPN_IPV4_UNICAST` `loc-rib` stays empty, and every VRF reports `paths-imported 0` while simultaneously reporting `l3vpn-ipv4-import-readiness: "Import ready"` and the correct `vpn-v4-import-route-targets`. Removing and re-adding an import route target does not re-trigger it. RT-based leaking on this image therefore needs a remote PE to reflect the VPN routes back, which is exactly what a single-device leaking topology (`tests/integration/vrf/31`-`35`) does not provide.

  One earlier claim in this document was wrong and is worth correcting explicitly: enabling `L3VPN_IPV4_UNICAST` on the default-instance BGP does **not** change the capabilities advertised on existing global-table sessions. ArcOS activates address families per neighbour, and on a live `bgp/01-ebgp-session` lab both sessions stayed `Established` with no reset and negotiated no additional address family after the instance-level AF was added.

* **ArcOS does have a working local inter-VRF leak, but it cannot carry netlab's model.** `static-route <pfx> next-hop-index <n> next-network-instance-name <vrf>` installs the route in the configuring VRF resolved through the named VRF, and traffic crosses: with the mirror-image entry in the other direction, two hosts in different VRFs ping each other 3/3 while a host in a third VRF stays unreachable. It accepts **host routes only** -- a shorter prefix is rejected with *"Expect /32 v4 prefix when configuring static route with next-network-instance option"*, and IPv6 likewise needs a /128. Since `vrfs.<name>.import/export` are prefix-based, the construct cannot express them, so nothing is rendered for it. This is the counter-example a reader would otherwise find and conclude the caveat above is wrong: the platform *can* leak between VRFs, it just cannot do it the way netlab asks.
* **Genuinely unsupported**, not merely untested (and therefore not declared in `features`): EVPN symmetric IRB / L3VNI -- but note the reason: a route-type-5 IP-prefix route **is** originated and can be imported; what never happens is advertising it to the EVPN peer, so only the L2VNI / distributed-anycast-gateway form is declared (see the EVPN symmetric IRB caveat above); OSPFv2 area-interface authentication; routing-policy **extended**-community operations of any kind (matching, setting or deleting -- `ext-community-set` exists in the model but was never verified; standard and large communities are supported, see the community caveat above); redistribution **into** an IGP (OSPF accepts no import source and IS-IS accepts only static; redistribution *into* BGP works for connected, static, OSPF and IS-IS); and the `bgp.session` and other dotted plugins.
* **BGP community operations in routing policies work for standard and large communities; extended communities are not supported.** Community lists become OpenConfig `bgp-defined-sets` (`community-set` / `large-community-set`) and their members may be literals or regular expressions. Matching is `conditions bgp-conditions match-community-set` / `match-large-community-set`; setting is `set-community method INLINE` for standard communities and `set-large-community method REFERENCE` for large ones (ArcOS has no inline form for large communities, so *netlab* materialises a `large-community-set` for the action to reference); deletion is the same reference with `options REMOVE`, applied to the `-additional` leaf so a statement that also sets communities does not collide with itself. `ext-community-set` exists in the model but is not exercised or declared, so matching, setting or deleting **extended** communities is refused rather than silently dropped.
  * One netlab shape has no faithful rendering: an ArcOS set has no per-member action, only a match option for the whole set (`ANY`/`ALL`/`INVERT`). A list whose entries all permit renders with `ANY`; the common "deny these, permit everything else" idiom renders the denied members with `INVERT`. Any other mix of permit and deny in one list is refused by a device quirk during topology transformation, because approximating a route-map match is the kind of error that shows up as traffic taking the wrong path rather than as a failure.
* **The BGP link-bandwidth extended community is not supported** (the `bgp.policy` plugin's **bgp.bandwidth** attribute, used for unequal-cost load balancing). There is no link-bandwidth leaf anywhere in the device's schema and no `send-community` knob under the BGP neighbor address family. Neither of the two ways to hand-write one produces a usable result, and they fail differently:
  * the **inline** form (`actions bgp-actions set-ext-community method INLINE ... inline ext-communities [ bandwidth:<asn>:<value> ]`) does reach the peer, which decodes it as a link-bandwidth community -- but the **bandwidth field is always zero** whatever value is configured. Measured with 100 and 12500000: the peer reported `Extended Community: LB:65000:0 (0 bps)` both times, while the ASN field did follow the configuration;
  * the **reference** form (an `ext-community-set` holding the same `bandwidth:<asn>:<value>` member, applied with `method REFERENCE`) commits, but **no extended community reaches the peer at all**.

  A link-bandwidth community advertising 0 bps would make a peer weight the path at zero, which is worse than sending nothing, so neither form is rendered and the attribute stays undeclared.
* **Spanning tree is Rapid PVST and nothing else, and it runs by default.** The `stp enabled-protocol` leaf's own value list is `NONE` or `RAPID_PVST`, so there is no 802.1D STP, no plain RSTP and no MSTP; `features.stp.supported_protocols` declares `pvrst` alone and *netlab* rejects every other protocol rather than mis-rendering it. A lab with no STP configuration at all already converges on a root and blocks one of two parallel links. Three further differences from *netlab*'s model are worth knowing:
  * **The only off switch is global.** Neither `stp interface <if>` nor `stp rapid-pvst vlan <id>` has an `enabled` leaf -- `stp enabled-protocol NONE` stops spanning tree on the whole device. `features.stp.enable_per_port` is therefore `False`, which makes *netlab* refuse a per-port request. *netlab* core does **not** gate the per-VLAN form, so `netsim/devices/arcos.py` refuses `vlans.<name>.stp.enable: False` during topology transformation; without that check the request would transform, render and deploy while the device kept running spanning tree on that VLAN. Use node-level `stp.enable: False` to stop STP everywhere.
  * **There is no bridge-wide priority** -- `stp rapid-pvst vlan <id> bridge-priority` is the only one -- so a node-level `stp.priority` is repeated on every VLAN the node knows.
  * **`stp.port_type` is not declared** and `bpdu-guard` is not rendered. Of the four *netlab* port types, `edge`/`auto`/`normal` map onto `stp interface <if> edge-port`, but `network` has no per-port equivalent (ArcOS bridge assurance is the global `stp global bridge-assurance`), and the feature flag is a bare boolean that cannot express a subset. `stp global bpdu-guard true` commits and appears in the running configuration, but `show stp global state bpdu-guard` still reads `false`, so nothing relies on it.

  `stp.port_priority` **is** rendered -- as ArcOS's 8-bit `port-priority` (0-240), which is *netlab*'s 4-bit value times 16, emitted per VLAN per interface because that is where ArcOS keeps it -- but **its effect on the root-port election has not been observed**. The leaf is accepted and shows up in operational state; no ArcOS topology has yet been built in which changing it moved the elected port, and the integration test that would demonstrate it (`stp/03-stp-port-priority`) is unreachable because it requires 802.1D. It is rendered rather than discarded because *netlab* does not feature-gate `stp.port_priority`, so the alternative is dropping it silently.
...
* VXLAN-encapsulated ARP traffic generated by ArubaCX has an invalid IP packet length and is rejected by Linux-based devices.
...
* Dell OS10 uses a so-called *Virtual Network* interface for VLANs that use VXLAN. There are several limitations when using such SVIs - OSPF is not supported, and gateways are limited to anycast (not VRRP)
* The *anycast* gateway is not supported on plain *ethernet* interfaces, so you need to use *VRRP* there.
