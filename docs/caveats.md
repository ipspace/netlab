...
* **The BGP link-bandwidth extended community is not supported** (the `bgp.policy` plugin's **bgp.bandwidth** attribute, used for unequal-cost load balancing). There is no link-bandwidth leaf anywhere in the device's schema and no `send-community` knob under the BGP neighbor address family. ArcOS *will* emit an LB extended community if one is hand-written as an `ext-community-set` member in the `bandwidth:<asn>:<value>` form, and a peer decodes it as a link-bandwidth community -- but the **bandwidth field is always zero** whatever value is configured (measured with 100 and 12500000; the peer reported `LB:65000:0 (0 bps)` in both cases, while the ASN field did follow the configuration). A link-bandwidth community that advertises 0 bps would make a peer weight the path at zero, so nothing renders it and the attribute stays undeclared.
