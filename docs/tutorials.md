...
In an empty directory create the [lab topology file](topology-overview.md) `lab.yml`:
...
Create **Vagrantfile**, **hosts.yml** (Ansible inventory file) and **ansible.cfg** (Ansible configuration file) with **[create-topology](create-topology.md)** tool:
...
The Ansible inventory created by **create-topology** tool contains enough information to configure interfaces and OSPF routing process. Here's the inventory information for *r1* (`host_vars/r1/topology.yml`):
...
To configure the lab devices, run **initial-config.ansible** playbook. We'll use the `dense` callback to minimize the amount of detritus generated by Ansible:

```
$ ANSIBLE_STDOUT_CALLBACK=dense initial-config.ansible
PLAY 1: DEPLOY DEVICE CONFIGURATION
task 3: r1 r2
task 7: r1 r2
```

If you want to inspect the deployed device configurations, you could use the `-v` (verbose) flag -- the playbook would print out the device configurations before they'd be deployed -- or **create-config.ansible** playbook to create configuration snippets in `config` directory.

## Connecting to Network Devices

After installing Ansible, you could use the **connect.sh** script to connect to network devices[^2] and inspect the OSPF neighbors and IP routing table:

[^2]: **connect.sh** script uses Ansible inventory to find the administrator username and password, device IP address and SSH port name.
...
* [Multiple BGP examples](module/bgp.md#examples)
