...
Let's start with two isolated VRFs: *red* and *blue*. We could ignore all details (route distinguishers and route targets); *netsim-tools* will auto-generate all required values.
...
*netsim-tools* use simple isolated VRFs as the default connectivity model:
...
* Attributes in YAML files generated by *netsim-tools* are sorted alphabetically (blue before red).
